...
#'
#' @description This function implements logistic regression adjusting for number of probes in enrichment analysis.
...
#' @details The implementation of this function is modified from goglm function in GOglm package.
#' @export
#' @import stats
#' @import org.Hs.eg.db
#' @import AnnotationDbi
#' @return A data frame contains gene set tests results.
#' @references Mi G, Di Y, Emerson S, Cumbie JS and Chang JH (2012) Length bias correction in Gene Ontology enrichment analysis using logistic regression. PLOS ONE, 7(10): e46128
...
methylglm <- function(cpg.pval, array.type = "450K", GS.list=NULL, GS.idtype = "SYMBOL", GS.type = "GO", minsize = 100, maxsize = 500){
  ...
  gene.pval = unlist(lapply(geneID.list, min))    ## for each gene, take the minimum p-value
  probes = unlist(lapply(geneID.list, length))   ## get number of probes for each gene
  ...
  for(i in 1:length(GS.list.sub)){
    ...
    glm.fit = glm(y ~ NegLogP + probes, family = "quasibinomial", data = df, control = list(maxit = 100))
    sumry = summary(glm.fit)
    gs.pval[i] = sign(coefficients(glm.fit)[[2]])*sumry$coef[ ,"Pr(>|t|)"][[2]]
  }
  gs.pval[gs.pval<=0] = 1
