...
#'
#' @description This function calls gometh or gsameth function in missMethyl package to adjust number of probes in gene set testing
...
#' @references Ligtenberg W (2017). reactome.db: A set of annotation maps for reactome. R package version 1.62.0.
#' @references Carlson M (2017). org.Hs.eg.db: Genome wide annotation for Human. R package version 3.5.0.
#' @references Hansen KD (2016). IlluminaHumanMethylation450kanno.ilmn12.hg19: Annotation for Illumina's 450k methylation arrays. R package version 0.6.0.
...
methylgometh <- function(cpg.pval, sig.cut, array.type = "450K", GS.list=NULL, GS.idtype = "SYMBOL", GS.type = "GO", minsize = 100, maxsize = 500){
  ...
  if(is.null(GS.list) & (GS.type=="GO"|GS.type=="KEGG")){
    ...
    res = gometh(sig.cpg = sig.cpg, all.cpg = names(cpg.pval), collection = GS.type,
                 array.type=array.type, plot.bias = FALSE, prior.prob = TRUE)
  ...
  if(is.null(GS.list) & GS.type=="Reactome"){
    cat(paste(GS.type, "are being tested...\n"))
    if(array.type=="450K")
      FullAnnot = getAnnotation(IlluminaHumanMethylation450kanno.ilmn12.hg19::IlluminaHumanMethylation450kanno.ilmn12.hg19)
    else
      FullAnnot = getAnnotation(IlluminaHumanMethylationEPICanno.ilm10b3.hg19::IlluminaHumanMethylationEPICanno.ilm10b3.hg19)
    ...
    gene.entrez = suppressMessages(select(org.Hs.eg.db, temp, columns = "ENTREZID", keytype = "SYMBOL")$ENTREZID)
    reactome.df = suppressMessages(select(reactome.db, gene.entrez, columns = "REACTOMEID", keytype = "ENTREZID"))
    ...
    reactome.list.sub = reactome.list[reactome.list.sizes>=minsize & reactome.list.sizes<=maxsize]

    res = gsameth(sig.cpg = sig.cpg, all.cpg = names(cpg.pval), collection = reactome.list.sub,
                  array.type=array.type, plot.bias = FALSE, prior.prob = TRUE)
    ...
    if(!is.null(GS.list) & GS.idtype!="ENTREZID"){
      ...
      GS.list.entrezid = suppressMessages( lapply(GS.list, function(x)
        return( select(org.Hs.eg.db, x, columns = "ENTREZID", keytype = GS.idtype)$ENTREZID )) )
    }
    ...
    GS.list.sub = GS.list.entrezid[GS.sizes>=minsize & GS.sizes<=maxsize]   ## filter gene sets by their sizes
    cat(length(GS.list.sub), "gene sets are being tested...\n")
    res = gsameth(sig.cpg = sig.cpg, all.cpg = names(cpg.pval), collection = GS.list.sub,
                  array.type=array.type, plot.bias = FALSE, prior.prob = TRUE)
    res = res[order(res[,3]),]
    cat("Done!\n")
    return(res)
